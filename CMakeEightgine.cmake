...
set(EIGHTGINE_WARNING_FLAGS "-Wall" "-Wextra" "-Wpedantic" "-Werror")
set(EIGHTGINE_SOURCES_TYPES "cpp" "hpp" "c" "h")
...
set(EIGHTGINE_VISIBILITY_HIDDEN C_VISIBILITY_PRESET hidden CXX_VISIBILITY_PRESET hidden VISIBILITY_INLINES_HIDDEN ON)
...
function(_eightgine_file)
    ...
    if(NOT ARG_MODULE_OR_EXECUTABLE_SOURCES_TYPES)
        set(ARG_MODULE_OR_EXECUTABLE_SOURCES_TYPES ${EIGHTGINE_SOURCES_TYPES})
    endif()
...
function(eightgine_configure_module_or_executable)
    ...
    if(ARG_MODULE_OR_EXECUTABLE_OPTIONS)
        target_compile_options("${DIRTY_ARG_MODULE_OR_EXECUTABLE_NAME}" ${ARG_MODULE_OR_EXECUTABLE_NAME_INTERNAL} ${ARG_MODULE_OR_EXECUTABLE_OPTIONS})
    endif()
endfunction()
...
function(eightgine_add_module)
    set(ONE_VALUE_ARGS
        ...
        MODULE_ALIAS
        MODULE_TYPE
        MODULE_SCOPE # configurable
        MODULE_DEFAULT_DEFINITIONS
        MODULE_DEFAULT_OPTIONS
        MODULE_LIB_DIR # configurable
        MODULE_BIN_DIR # configurable
        MODULE_DESTINATION_DIR # configurable
    )
    set(MULTI_VALUE_ARGS
        MODULE_INCLUDE_DIR # configurable
        MODULE_SOURCES_DIR MODULE_SOURCES_FILES # configurable
        MODULE_DEFINITIONS # configurable
        MODULE_PROPERTIES # configurable
        MODULE_OPTIONS # configurable
    )
    cmake_parse_arguments("ARG" "" "${ONE_VALUE_ARGS}" "${MULTI_VALUE_ARGS}" ${ARGN})
    ...
    if(NOT ARG_MODULE_DEFAULT_DEFINITIONS STREQUAL "OFF")
        ...
        if(ARG_MODULE_TYPE STREQUAL "INTERFACE")
            ...
            list(APPEND MODULE_DEFAULT_PROPERTIES ${EIGHTGINE_VISIBILITY_HIDDEN})
    ...
    if(EIGHTGINE_PLATFORM_LINUX)
        list(APPEND MODULE_DEFAULT_PROPERTIES BUILD_WITH_INSTALL_RPATH TRUE INSTALL_RPATH "${EIGHTGINE_RPATH}")
    elseif(EIGHTGINE_PLATFORM_MACOS)
        list(APPEND MODULE_DEFAULT_PROPERTIES INSTALL_NAME_DIR "@rpath" BUILD_WITH_INSTALL_RPATH TRUE INSTALL_RPATH "${EIGHTGINE_RPATH}")
    endif()

    if(NOT ARG_MODULE_DEFAULT_OPTIONS STREQUAL "OFF")
        set(MODULE_DEFAULT_OPTIONS ${EIGHTGINE_WARNING_FLAGS})
    endif()
...
function(eightgine_add_executable)
    set(ONE_VALUE_ARGS
        EXECUTABLE_NAME
        EXECUTABLE_DESTINATION_DIR # configurable
        EXECUTABLE_DEFAULT_OPTIONS
    )
    set(MULTI_VALUE_ARGS
        EXECUTABLE_INCLUDE_DIR # configurable
        EXECUTABLE_SOURCES_DIR EXECUTABLE_SOURCES_FILES # configurable
        EXECUTABLE_DEFINITIONS # configurable
        EXECUTABLE_PROPERTIES # configurable
        EXECUTABLE_OPTIONS # configurable
    )
    cmake_parse_arguments("ARG" "" "${ONE_VALUE_ARGS}" "${MULTI_VALUE_ARGS}" ${ARGN})
    ...
    if(NOT ARG_EXECUTABLE_DEFAULT_OPTIONS STREQUAL "OFF")
        set(EXECUTABLE_DEFAULT_OPTIONS ${EIGHTGINE_WARNING_FLAGS})
    endif()
...
function(eightgine_add_dependency)
    set(ONE_VALUE_ARGS
        ...
        DEPENDENCY_NAME
        DEPENDENCY_SCOPE
        DEPENDENCY_LIB_DIR # configurable
    )
    set(MULTI_VALUE_ARGS
        DEPENDENCY_INCLUDE_DIR # configurable
    )
    cmake_parse_arguments("ARG" "" "${ONE_VALUE_ARGS}" "${MULTI_VALUE_ARGS}" ${ARGN})
