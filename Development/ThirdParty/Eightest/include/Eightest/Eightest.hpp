...
#define EXPECT(msg, ...) eightest::global.check(__VA_ARGS__, this, msg)
#define ASSERT(msg, ...) if(!EXPECT(msg, __VA_ARGS__)) return

// Tests Execution
#define EXECUTE_TEST(name, ...) eightest::global.execute_test(name)
#define EXECUTE_MODULE(name, ...) eightest::global.execute_module(name)
#define EXECUTE_ALL(...) eightest::global.execute_all()

// Show stat info
#define TESTING_STAT(...) eightest::global.stat()

// Will catch exception and show by text printer
#define TRY_CATCH(...) eightest::global.try_catch([]{ (void)(__VA_ARGS__); })

#define STAT_HANDLER(...) eightest::global.stat_handler = __VA_ARGS__
