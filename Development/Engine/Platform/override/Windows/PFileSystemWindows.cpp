...
struct PFileSystemWindows : public PFileSystemInterface
{
    ...
    std::filesystem::path ProjectModulesDir() override;
    std::filesystem::path ProjectPlugInModulesDir() override;
    std::filesystem::path ProjectResourcesDir() override;
};

EIGHTGINE_REGISTER_PLATFORM(PFileSystemWindows)
...
std::filesystem::path PFileSystemWindows::ProjectModulesDir()
{
    return ProjectExecutableDir().parent_path() / EIGHTGINE_MODULES_DIR_NAME;
}

std::filesystem::path PFileSystemWindows::ProjectPlugInModulesDir()
{
